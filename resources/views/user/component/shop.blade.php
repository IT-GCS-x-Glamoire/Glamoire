  ...
  <div class="container-fluid">
    <div class="shadow-sm border border-black rounded-md py-2 py-md-3 my-2 my-md-3">
      <div class="d-flex gap-2 pl-2">
        ...
        <a href="#" class="text-black text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px]">{{ $category }}</a>
      </div>
    </div>
  </div>
    ...
    <div class="row">
      ...
      <div class="col-lg-2 col-md-3 d-none d-md-block">
        ...
        <div class="border border-black shadow-md rounded-md md:mb-0 lg:mb-0 xl:mb-0 py-1 px-3">
          ...
          <form action="{{ route('shop.category', ['category' => $category]) }}" method="GET" id="form-filter-category">
            <!-- Categories Start -->
            <div class="border-bottom pb-2">
              <h5 class="font-weight-semi-bold text-[#183018] mb-2">{{ $category }}</h5>
              @for ($a=1; $a <= 8; $a++)
                <div class="d-flex align-items-center">
                  <a href="{{ route('shop.category.sub', ['category' => 'category' . $a, 'subcategory' => 'subcategory' . $a]) }}" class="text-[7px] md:text-[9px] lg:text-[11px] xl:text-[13px]">Sub Kategori {{ $a }}</a>
            ...
            <div class="border-bottom mb-4 pb-4">
              ...
              <div>
                <div class="price-range-container">
                  <div>
                    <label for="min_price" class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Harga Terendah: </label><br>
                    <input class="w-full" type="range" id="min_price" name="min_price" min="0" max="500000" step="10000" value="0" oninput="updatePriceRange()"/>
                    <span id="min_price_value" class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Rp0</span>
                  </div>
  
                  <div>
                    <label for="max_price" class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Harga Tertinggi: </label><br>
                    <input class="w-full" type="range" id="max_price" name="max_price" min="500000" max="1000000" step="10000" value="1000000" oninput="updatePriceRange()"/>
                    <span id="max_price_value" class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Rp1,000,000</span>
                  </div>
              ...
              <div class="row">
                <div class="flex w-full align-items-center justify-content-between mb-2 mb-my-4">
                  <div class="flex justify-content-center align-items-center">
                    <h1 class="text-[12px] md:text-[12px] lg:text-[14px] xl:text-[16px] text-[#183018] ml-2 ml-md-0">{{ $category }}</h1>
                  </div>
                  ...
                  <div class="dropdown flex ml-auto">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="triggerId">
                      <form action="{{ route('shop.category', ['category' => $category]) }}" method="GET" id="form-filter-sortby">
                        <input type="hidden" name="sort" id="sort-input">
                        <a class="dropdown-item text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px] hover:bg-[#183018] hover:text-white" href="#" data-value="newest">Terbaru</a>
                        <a class="dropdown-item text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px] hover:bg-[#183018] hover:text-white" href="#" data-value="popular">Terpopuler</a>
                        <a class="dropdown-item text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px] hover:bg-[#183018] hover:text-white" href="#" data-value="highest">Harga Tertinggi</a>
                        <a class="dropdown-item text-[8px] md:text-[10px] lg:text-[12px] xl:text-[14px] hover:bg-[#183018] hover:text-white" href="#" data-value="lowest">Harga Terendah</a>
                      </form>
                    </div>
                  </div>
                </div>
             ...
             <div>
               <div class="price-range-container">
                 <div class="grid gap-1">
                   <label for="min_price" class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Harga Terendah: </label>
                   <input class="w-full" type="range" id="min_price-mobile" name="min_price" min="0" max="500000" step="10000" value="0" oninput="updatePriceRange()"/>
                   <span id="min_price_value" class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Rp0</span>
                 </div>
 
                 <div class="grid gap-1">
                   <label for="max_price" class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Harga Tertinggi: </label>
                   <input class="w-full" type="range" id="max_price-mobile" name="max_price" min="500000" max="1000000" step="10000" value="1000000" oninput="updatePriceRange()"/>
                   <span id="max_price_value" class="text-[10px] md:text-[10px] lg:text-[12px] xl:text-[14px]">Rp1,000,000</span>
                 </div>
...
<script>
  ...
  document.querySelectorAll('.dropdown-item').forEach(function(item) {
    item.addEventListener('click', function(e) {
      ...
      var sortValue = this.getAttribute('data-value'); // Get the value from the clicked link's data-value attribute
      document.getElementById('sort-input').value = sortValue; // Set the value in the hidden input field
      document.getElementById('form-filter-sortby').submit(); // Submit the form
    });
  });
